...
import { markMessageRead, markMessageUnread, deleteMessage } from "@/actions/contact";
import { exportMessagesCSV } from "@/actions/waitlist";
...
export function MessagesClient({ messages }: { messages: Message[] }) {
  ...
  const filtered = useMemo(() => {
    return messages.filter((m) => {
      const matchFilter =
        filter === "all" || (filter === "unread" ? !m.isRead : m.isRead);
  ...
  const handleExport = async () => {
    const result = await exportMessagesCSV();
    if (result.csv) {
      downloadCsv(result.csv, `messages-${new Date().toISOString().split("T")[0]}.csv`);
    }
  };
    ...
    <div>
          ...
          {(["all", "unread", "read"] as Filter[]).map((f) => (
            ...
            >
              ...
              <span className="text-xs opacity-60">
                ({f === "all" ? messages.length : f === "unread" ? messages.filter(m => !m.isRead).length : messages.filter(m => m.isRead).length})
              </span>
            </button>
          ))}
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="p-12 rounded-xl bg-[#0f1a14] border border-white/10 text-center">
          <p className="text-[#6a8a7a]">{search ? "Aucun résultat" : "Aucun message"}</p>
        </div>
      ) : (
        <div className="flex flex-col gap-2">
          {filtered.map((msg) => (
            <div
              ...
              onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); handleSelect(msg); } }}
            ...
            >
              ...
              <div className={`flex-1 min-w-0 ${msg.isRead ? "pl-6" : ""}`}>
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-white text-sm font-medium">{msg.name}</span>
                  <span className="text-[#6a8a7a] text-xs">{msg.email}</span>
                  <span className="text-[#6a8a7a] text-xs ml-auto">
                    {new Date(msg.createdAt).toLocaleDateString("fr-FR")}
                  </span>
                </div>
                <p className="text-[#6a8a7a] text-sm truncate">{msg.message}</p>
              </div>
              <div className="flex items-center gap-1 shrink-0" onClick={(e) => e.stopPropagation()}>
                <a
                  href={`mailto:${msg.email}?subject=Re: Votre message SquamaTracker`}
                  title="Répondre"
                ...
                <button
                  onClick={() =>
                    startTransition(() =>
                      msg.isRead ? markMessageUnread(msg.id) : markMessageRead(msg.id)
                    )
                  }
                  disabled={isPending}
                ...
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d={msg.isRead ? "M3 8l7.89 5.26a2 2 0 002.22 0L21 8" : "M5 13l4 4L19 7"} />
                  </svg>
                </button>
                <button
                  onClick={() => handleDelete(msg.id)}
                  title={deleteConfirm === msg.id ? "Confirmer" : "Supprimer"}
          ...
          <div ref={modalRef} role="dialog" aria-modal="true" aria-label="Détail" tabIndex={-1} className="w-full max-w-lg bg-[#0f1a14] rounded-2xl border border-white/10 p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <p className="text-[#6a8a7a] text-sm">{selected.email}</p>
              ...
              <div className="flex items-center gap-2">
                <a
                  href={`mailto:${selected.email}?subject=Re: Votre message SquamaTracker`}
